          ...
          {loading ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[...Array(4)].map((_, i) => (
                <div
                  key={i}
                  className="bg-white rounded-2xl p-5 h-24 animate-pulse border border-slate-100"
                />
              ))}
            </div>
          ) : (
            ...
            <div className="space-y-3">
              {history.slice(0, 5).map((item, idx) => {
                ...
                return (
                  <div
                    key={item._id}
                    className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 transition-colors animate-slide-up"
                  ...
                  >
                    ...
                    <span
                      className={`px-2.5 py-1 rounded-full text-xs font-bold ${scoreBg} ${scoreColor}`}
                    >
                      {item.score}%
                    </span>
                  </div>
        ...
        {!loading && history.length === 0 && (
          <div className="bg-white rounded-2xl shadow-card border border-slate-100 p-12 text-center animate-fade-in">
            ...
            <p className="text-slate-500 text-sm">
              Analyze your first resume to see stats here.
            </p>
          </div>
        )}
      </div>
