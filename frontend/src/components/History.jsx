  ...
  return (
    <div className="flex items-center gap-3">
      <div className="w-24 bg-slate-200 rounded-full h-1.5">
        <div
          className={`${cfg.bar} h-1.5 rounded-full progress-bar`}
          style={{ width: `${score}%` }}
        />
      </div>
      <span
        className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${cfg.bg} ${cfg.text}`}
      >
        {score}%
      </span>
    </div>
...
const History = () => {
  ...
  const avgScore = history.length
    ? Math.round(history.reduce((a, i) => a + i.score, 0) / history.length)
    : 0;
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 px-4 py-8">
      <div className="max-w-6xl mx-auto">
        ...
        {history.length > 0 && (
          ...
          >
            {[
              ...
              <div
                key={label}
                className={`${bg} rounded-2xl p-5 text-center border border-white`}
              >
        ...
        {loading ? (
          ...
          <div
            className="bg-white rounded-2xl shadow-card border border-slate-100 overflow-hidden animate-slide-up"
          ...
          >
            ...
            {history.map((item, idx) => (
              <div
                key={item._id}
                className={`grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0 animate-slide-up`}
                style={{ animationDelay: `${idx * 50}ms` }}
              >
                <div className="col-span-4">
                  <p className="text-sm font-medium text-slate-900 truncate">
                    {item.jobRole || "Unknown Role"}
                  </p>
                  <a
                    href={`${import.meta.env.VITE_API_URL || "http://localhost:8000"}${item.resumeUrl}`}
                    target="_blank"
